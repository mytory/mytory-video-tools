...
音声圧縮ツールがAACやOpusを含む、FFmpegが対応するすべての音声形式を受け付けるようになりました。読み込めないファイルを選ぶとエラーが表示され、ドロップ領域が復元されます。エラー後もファイルを選び直せます。

---

## 1. 特徴

1. **サイドバータブナビゲーション**: 8つのコアメディアユーティリティをスタイリッシュなダークテーマのダッシュボード内で切り替えられます。
...
| 🗜️ **音声圧縮 (Audio Compressor)** | FFmpegが対応するあらゆる形式の音声ファイルを高音質MP3に変換。CBR/VBRエンコードに対応。 |
| 📸 **フレームキャプチャ (Frame Capture)** | 単一フレームキャプチャ、間隔を指定した一括抽出、感度調整可能な自動シーン検出。キャプチャ画像のEXIFには実際のキャプチャ時刻を記録。 |
...
### ダウンロード
*   [GitHub Releases](https://github.com/mytory/mytory-video-tools/releases) ページにアクセスし、お使いのOSに合ったパッケージをダウンロードします。
    *   **Windows:** `.exe`（インストーラー）
    *   **macOS:** `.dmg`（ディスクイメージ） — Intel (x64) / Apple Silicon (arm64)
...
* `.env` の例:
    ...
    ```
* `.github/workflows/release.yml` の `CSC_LINK` / `CSC_KEY_PASSWORD` は、GitHub Actions 環境で P12 ベースの署名に使用されます。
