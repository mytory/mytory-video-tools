...
音频压缩器现在接受 FFmpeg 支持的所有音频格式，包括 AAC 和 Opus。遇到无法读取的文件时会显示错误并恢复拖放区域；发生错误后仍可重新选择文件。

---

## 1. 功能特性

1. **侧边栏标签导航**：在时尚的深色主题仪表板中切换 8 个核心媒体工具。
...
| 🗜️ **音频压缩器 (Audio Compressor)** | 将 FFmpeg 支持的任意格式音频转换为高质量 MP3，并支持 CBR/VBR 编码。 |
| 📸 **帧捕获 (Frame Capture)** | 单帧捕获、按间隔批量提取、灵敏度可调的自动场景检测。捕获图像的 EXIF 会记录实际捕获时刻。 |
