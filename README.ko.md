...
오디오 압축기가 AAC와 Opus를 포함해 FFmpeg가 지원하는 모든 오디오 형식을 받습니다. 읽을 수 없는 파일을 선택하면 오류를 표시하고 드롭 영역을 복구하며, 오류 후에도 파일을 다시 선택할 수 있습니다.

---

## 1. 주요 특징

1. **사이드바 탭 네비게이션**: 8가지 핵심 미디어 유틸리티를 하나의 세련된 다크 테마 대시보드 안에서 자유롭게 전환하며 사용할 수 있습니다.
...
| 🗜️ **오디오 압축기 (Audio Compressor)** | FFmpeg가 지원하는 모든 형식의 오디오를 고품질 MP3로 변환합니다. CBR/VBR 인코딩을 지원합니다. |
| 📸 **장면 캡처 (Frame Capture)** | 프레임 단위 탐색 캡처, 지정 간격(Batch) 연속 추출 및 민감도 조절을 통한 장면 전환(Scene Detection) 자동 추출 기능을 제공합니다. 캡처 이미지에는 실제 캡처 시각을 EXIF로 기록합니다. |
| 🔄 **확장자 변환기 (Remuxer)** | 재인코딩 과정 없이 고속으로 비디오의 컨테이너 포맷(MP4, MKV, MOV)만 변경합니다. |
| 🔗 **영상 합치기 (Video Joiner)** | 자동 호환성 처리와 함께 여러 비디오 파일을 하나로 합칩니다. 드래그로 순서를 변경하고 호환성을 한눈에 확인한 후 한 번에 합치세요. 프레임레이트 차이는 스마트 재인코딩으로 처리합니다. 분할한 구간을 다시 조립할 때 사용합니다. |
